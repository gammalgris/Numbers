...
/**
 * An implementation of an algorithm that approximates the cosine function
 * (see <a href="https://en.wikipedia.org/wiki/Sine_and_cosine">sine and cosine</a>).<br>
 * <br>
 * sin(x) = &Sigma; (-1)<sup>n</sup> / ( 2 * n)! * x<sup>(2 * n)</sup><br>
 * <br>
 ...
 */
public class CosineApproximation implements TernaryOperation<Number, Result<Number>> {

    /**
     * The default constructor.
     */
    public CosineApproximation() {

        super();
    }

    /**
     * Calculates an approximation for the cosine function.
     *
     * @param x
     *        an input value
     * @param iterations
    ...
    private Number next(ProcessingDetails processingDetails, Number n, Number x) {
        ...
        Number evenNumber = TWO.multiply(n);

        Number term1 = MINUS_ONE.exponentiate(processingDetails, n);
        Number term2 = evenNumber.factorial();
        Number term3 = x.exponentiate(processingDetails, evenNumber);
